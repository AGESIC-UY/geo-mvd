...
public class ContBusqueda {
	...
	public ContBusqueda(ContPpal p) {
		...
		ImageIcon icon = GuiUtils.loadIcon("Search16.gif");
		if (icon == null){
			logger.info("--------> No se pueden encontrar imagenes, la interfaz no tendra iconos!!! <----------");
		}
		
		tmp.setIconImage(icon == null ?  null : icon.getImage());
