...
/**
 * Copia del DefaultAttributeType para poder invocarlo desde
 * nuestra FeatureFactory
 * 
 * @author agrassi
 *
 */
public class MixtoDefaultAttributeType implements AttributeType {
     ...
     *       at this time.
     */
    protected MixtoDefaultAttributeType(String name, Class<?> type, boolean nillable, int min, int max,
    ...
    protected MixtoDefaultAttributeType(String name, Class<?> type, boolean nillable, int min, int max,
            Object defaultValue) {
    	this(name,type,nillable,min,max,defaultValue,Filter.EXCLUDE);
    }

    protected MixtoDefaultAttributeType(String name, Class<?> type, boolean nillable,
                                   Object defaultValue) {
    	this(name, type, nillable, 1, 1,  defaultValue,Filter.EXCLUDE);
    }

    protected MixtoDefaultAttributeType(AttributeType copy) {
        this.name = copy.getLocalName();
        this.type = copy.getBinding();
    ...
    public Object duplicate(Object src) throws IllegalAttributeException {
        ...
        if (src instanceof Map) {
            ...
            for (Iterator<?> i = map.entrySet().iterator(); i.hasNext();) {
                ...
                copy.put((Object)entry.getKey(), duplicate(entry.getValue()));
            }
    ...
    public boolean equals(Object other) {
        ...
        if (!name.equals(att.getLocalName())) {
            return false;
        }

        if (!type.equals(att.getBinding())) {
            return false;
        }
